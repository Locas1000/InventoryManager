...
interface Comment {
    id: number;
    userId: number; // 🟢 Added this so we can check ownership
    text: string;
    createdAt: string;
    userName: string;
}
...
export default function DiscussionBoard({ inventoryId }: Props) {
    ...
    // Get the logged-in user so we know which buttons to show
                        ...
                        <div key={comment.id} className="card border-0 shadow-sm">
                          {Number(currentUserId) === Number(comment.userId) && (
                            ...
                            <div className="card-body py-2">
                                {/* 🟢 If editing, show the textarea. Otherwise, show the Markdown. */}
